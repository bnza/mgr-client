...
export function useCreateValidation(
  parent?: ResourceParent<'individual'> | ResourceParent<'analysis'>,
) {
  type RequestBody = PostCollectionRequestMap['/api/data/analyses/individuals']

  const { key: parentKey, iri: parentIri } = useResourceParent(parent)
  const getEmptyModel = () =>
    ({
      subject: parentKey.value === 'individual' ? parentIri.value : undefined,
...
export function useUpdateValidation(
  params: Ref<
    | OperationPathParams<'/api/data/analyses/individuals/{id}', 'get'>
    | undefined
  >,
) {
