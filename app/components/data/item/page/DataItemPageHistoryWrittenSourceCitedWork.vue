<script setup lang="ts">
import useResourceUiStore from '~/stores/useResourceUiStore'
import type { GetItemResponseMap } from '~~/types'

const path = '/api/data/history/written_sources_cited_works/{id}' as const
type GetItemResponse = GetItemResponseMap[typeof path]

const { tab } = storeToRefs(useResourceUiStore(path))

const redirectToCollectionPath = useRedirectToCollectionPath(path)
</script>

<template>
  <data-item-page :path identifier-prop="id">
    <template #default="{ item }: { item: GetItemResponse }">
      <lazy-data-item-form-info-history-written-source-cited-work :item />
      <v-tabs v-model="tab" background-color="transparent">
        <v-tab value="writtenSource">written source</v-tab>
      </v-tabs>
      <v-tabs-window v-model="tab">
        <v-tabs-window-item
          value="writtenSource"
          data-testid="tab-window-written-source"
        >
          <data-item-page-history-written-source
            v-if="item.writtenSource"
            :iri="item.writtenSource['@id']"
          />
        </v-tabs-window-item>
      </v-tabs-window>
    </template>
    <template #dialogs="{ refetch }">
      <data-dialog-delete-history-written-source-cited-work
        @refresh="redirectToCollectionPath()"
      />
      <data-dialog-update-history-written-source-cited-work
        @refresh="refetch()"
      />
    </template>
  </data-item-page>
</template>
