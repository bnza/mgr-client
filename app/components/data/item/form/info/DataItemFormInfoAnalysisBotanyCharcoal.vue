...
withDefaults(
  defineProps<{
    item: GetItemResponseMap['/api/data/analyses/botany/charcoals/{id}']
    readLink?: boolean
  }>(),
  {
    readLink: true,
  ...
  <data-item-form-read>
    <v-row>
      <v-col cols="6" class="px-2">
        <v-text-field :model-value="item.subject?.code" label="charcoal">
          <template v-if="item.subject?.['@id']" #append-inner>
            <data-item-info-box-botany-charcoal
              :iri="item.subject?.['@id']"
              :read-link
            />
          </template>
