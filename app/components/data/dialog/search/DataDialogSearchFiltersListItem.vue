  ...
  <v-list-item data-testid="data-dialog-search-filters-list-item">
    <template #default>
      <v-container class="h-90">
        <v-row class="align-center justify-space-evenly" density="compact">
          ...
          <v-col cols="3">
            <v-text-field readonly :model-value="filter.propertyLabel" variant="solo-filled" label="property" flat />
          </v-col>
          <v-col cols="3">
            <v-text-field readonly :model-value="filter.operationLabel" label="operands" variant="solo-filled" flat />
          </v-col>
          <component :is="operandsComponent" :model-value="filter.operands" :path="filter.componentKey === 'Vocabulary' ? filter.path : undefined" readonly valid />
        </v-row>
      </v-container>
    </template>
  </v-list-item>
