...
const path = '/api/data/vocabulary/history/plants'

const { appPath } = useResourceConfig(path)
const { deleteDialogState } = storeToRefs(
  useResourceDeleteDialogStore('/api/vocabulary/history/plants/{id}'),
)
// const { updateDialogState } = storeToRefs(
//   useResourceUpdateDialogStore('/api/vocabulary/history/plants/{id}'),
// )
  ...
  <data-collection-table :path @acl="acl = { ...acl, ...$event }">
    ...
    <template #dialogs="{ refetch }">
      ...
      <data-dialog-delete-vocabulary
        path="/api/vocabulary/history/plants/{id}"
        @refresh="refetch()"
      />
