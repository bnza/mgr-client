...
const { deleteDialogState } = storeToRefs(
  useResourceDeleteDialogStore('/api/data/paleoclimate_sampling_sites/{id}'),
)
const { updateDialogState } = storeToRefs(
  useResourceUpdateDialogStore('/api/data/paleoclimate_sampling_sites/{id}'),
)
...
<template>
  <data-collection-table :path @acl="acl = { ...acl, ...$event }">
    ...
    <template #dialogs="{ refetch }">
      ...
      <data-dialog-update-paleoclimate-sampling-site @refresh="refetch()" />
    </template>
