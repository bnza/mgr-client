...
const { deleteDialogState } = storeToRefs(
  useResourceDeleteDialogStore('/api/data/analyses/botany/charcoals/{id}'),
)
...
<template>
  <data-collection-table :path :parent-id @acl="acl = { ...acl, ...$event }">
    ...
    <template #[`item.item.code`]="{ item }">
      <data-item-info-box-span-botany-charcoal
        :iri="item.subject['@id']"
    ...
    <template #dialogs="{ refetch }">
      ...
      <data-dialog-create-analysis-botany-charcoal
        ...
        @refresh="refetch()"
      />
      <data-dialog-delete-analysis-botany-charcoal @refresh="refetch()" />
