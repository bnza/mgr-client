...
export const useOpenApiStore = (fetchedSpec?: OpenAPIV3_1.Document) =>
  defineStore('openapi', () => {
    ...
    const findRelatedApiResourcePath = (
      ...
      const found = Object.entries(openApiSpec.paths).find(
        ([path, pathItem]) => {
          const operation = pathItem?.get
          if (!operation) return false

          // Match by tag AND response schema
          const hasSameTag = operation.tags?.some((tag) =>
