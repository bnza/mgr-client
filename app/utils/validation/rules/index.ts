...
type CreateSiteRules = ReglePartialRuleTree<
  ValidationRequestBody<'create', '/api/sites'>
>
type CreateSiteUserPrivilegesRules = ReglePartialRuleTree<
  ValidationRequestBody<'create', '/api/site_user_privileges'>
>
type UpdateSiteUserPrivilegesRules = ReglePartialRuleTree<
...
const rules = {
  ...
  update: {
    '/api/site_user_privileges/{id}':
      site_user_privileges.update as ComputedRef<UpdateSiteUserPrivilegesRules>,
  },
} as const
...
export function getRules<
  ...
  P extends ValidationMethodToPath<M>,
>(
  method: M,
  path: P,
...
export function getRules(method: ValidatedHttpMethod, path: string) {
  const isValidKey = (value: unknown): value is keyof typeof rules =>
  ...
  if (!pathRules) {
    throw new Error(
      `No validation rules found for method: ${method}, path: ${path}`,
    )
  }
